...
df = pd.read_csv('BabyCry4/babycry_extracted.csv')
...
for index, row in train_df.iterrows():
    if row['label'] == 1:
        ...
        cnt = 0
        for a in augments:
            audio, sr = librosa.load('BabyCry4/' + row['audio'])
            audio = augment_data(a, audio)
            audio_file = row['audio'][:-4] + '_aug' + str(cnt) +'.wav'
            cnt = cnt + 1
            sf.write('BabyCry4/' +  audio_file, np.ravel(audio), sr)
...
train_df.to_csv('BabyCry4/train.csv', index=False)